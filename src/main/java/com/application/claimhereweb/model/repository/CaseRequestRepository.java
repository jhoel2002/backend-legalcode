...
public interface CaseRequestRepository extends JpaRepository<CaseRequest, Long> {
  ...
  Page<CaseRequest> findAllByCustomerId(Long customerId, Pageable pageable);

  @Query("""
          SELECT cr FROM CaseRequest cr
          JOIN FETCH cr.lawyer l
          JOIN FETCH cr.customer c
          JOIN FETCH c.user u
          WHERE cr.code = :code
      """)
  Optional<CaseRequest> findCaseRequestWithDetails(@Param("code") String code);
